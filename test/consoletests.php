...
OutputFactory::Create(Console::class)->Write("this is from an object ");
OutputFactory::Create(Console::class)->WriteLn("created using " . OutputFactory::class);
sleep(3);

$c0 = new Console();
$c0->Clear();
$c0->DrawLn('%',80);
$c0->WriteLn("WELCOME THIS IS A UNIT TEST BUT ITS VISUAL");
$c0->WriteLn("SO THE ONLY WAY TO KNOW IF THE TESTS PASSED");
$c0->WriteLn("IS FOR YOU TO WATCH THE SCREEN");
$c0->DrawLn("%",80);
sleep(3);
...
sleep(2);
$c1->WriteLn("World");
$c1->PrependOutput("\t%LRED(--- PREPEND --->)\t");
$c1->AppendOutput("\t%GREEN(<--- APPEND ---)");
$c1->WriteLn("Check 123");
$c1->PrependOutput("\t--- PREPEND --->\t");
$c1->AppendOutput("\t<--- APPEND ---");
$c1->WriteLn("%YELLOW(Check 123)");
$c1->WriteLn("%RED(A)%WHITE(M)%BLUE(E)%RED(R)%WHITE(I)%BLUE(C)%RED(A)%WHITE(!)");
sleep(3);

$c2 = new Console();
$c2->Clear();
$c2->WriteLn("base64 encoded:");
$c2->WriteLn("base64 encoded", ['encoding' => OutputEncodingType::Base64]);
$c2->WriteLn(" ");
$c2->WriteLn("Gzipped:");
$c2->WriteLn("gzipped", ['compression' => OutputCompressionType::Gzip]);
$c2->WriteLn(" ");
sleep(3);

$c3 = new Console();
$c3->Clear();
$c3->Write("%RED(this is red) and %GREEN(this is green) and %YELLOW(this is yellow) and %BLUE(this is blue)");
$c3->Write("and %MAGENTA(this is magenta) and %CYAN(this is cyan) and %LGRAY(this is light gray) and %DGRAY(this is dark gray)");
$c3->Write("and %LRED(this is light red) and %LGREEN(this is light green) and %LYELLOW(this is light yellow)");
$c3->Write("and %LBLUE(this is light blue) and %LMAGENTA(this is light magenta) and %LCYAN(this is light cyan)");
$c3->WriteLn("and last but not least... %WHITE(this is white)!");
sleep(3);

$c4 = new Console();
$c4->Clear();
for($i = 10; $i < 100; $i=$i+10) {
    $c4->DrawLn("%RED(%)%BLUE(%)",$i);
}
sleep(3);

$c6 = new Console();
$c6->OutputStream(STDERR);
$c6->WriteLn("This is to stderr");
$c6->OutputStream(STDOUT);
$c6->WriteLn("This is to stdout");
sleep(3);
